...
def oauth(request):
    ...
    print('카카오계정 프로필 : ',kakao_profile)
    kakao_nickname = kakao_profile.get('nickname')
    print('카카오계정 닉네임 : ',kakao_nickname)
    kakao_profile_image = kakao_profile.get('profile_image_url')
    print('카카오계정 프로필이미지 url : ',kakao_profile_image)
    ...
    return redirect('/')
    # return HttpResponse(f'''code : {code}, token json:{token_json}', 닉네임:{kakao_nickname}, ''')